...
const StoryboardBoard: React.FC<StoryboardBoardProps> = ({
    ...
    return (
        <div className={`animate-in fade-in duration-700 ${className}`}>
            ...
            {/* Board Container */}
            <div className={`p-8 pb-32 ${layout === 'grid' ? 'grid grid-cols-3 gap-x-4 gap-y-10' : 'flex flex-col gap-8 max-w-5xl mx-auto'}`}>
                {itemsToRender.map((item, index) => (
                    <div key={item.id} className="relative group/shot">
                        {/* Insertion BEFORE (Only for the first item in the whole board) */}
                        {index === 0 && (
                            <div className={`absolute ${layout === 'grid' ? '-left-6 top-1/2 -translate-y-1/2' : 'left-1/2 -top-4 -translate-x-1/2'} z-20 group/insert opacity-0 group-hover/shot:opacity-100 transition-opacity`}>
                                <button
                                    onClick={() => setInsertAnchor(0)}
                                    className="w-6 h-6 flex items-center justify-center bg-[#D4AF37] text-black rounded-full text-lg font-bold shadow-lg hover:scale-110 active:scale-95 transition-all"
                                    title="在开头插入"
                                >
                                    +
                                </button>
                        ...
                        {/* Insertion AFTER (Minimalist + sign between items) */}
                        <div className={`absolute ${layout === 'grid' ? '-right-5 top-1/2 -translate-y-1/2' : 'left-1/2 -bottom-4 -translate-x-1/2'} z-20 group/insert opacity-0 group-hover/shot:opacity-100 transition-opacity`}>
                            <button
                                onClick={() => setInsertAnchor(index + 1)}
                                className="w-6 h-6 flex items-center justify-center bg-[#D4AF37] text-black rounded-full text-lg font-bold shadow-lg hover:scale-110 active:scale-95 transition-all"
                                title="在此后插入"
                            >
                                +
            ...
            {insertAnchor !== null && (
                <InsertShotModal
                    ...
                    onInsert={(desc) => onInsertShot(insertAnchor, desc)}
                />
            )}
